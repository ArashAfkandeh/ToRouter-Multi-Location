...
برنامه ToRouter یک ابزار قدرتمند مدیریت پروکسی‌های Tor است که یک سرور واحد را به چندین لوکیشن (بیش از ۵۰ لوکیشن) با IP و کشور خروجی متفاوت و قابل تنظیم تبدیل می‌کند.این ابزار با پشتیبانی از چندین تونل مستقل، پنل مدیریت تحت وب، چرخش خودکار مسیرهای هر تونل، مانیتورینگ زنده، CLI، SQLite و API منعطف، راهکاری ایده‌آل برای مدیریت حرفه‌ای مسیرهای Tor و حفظ حریم خصوصی است.
...
- 🔐 ورود محافظت‌شده و امکان تغییر مسیر پایه (Web Base Path) برای مخفی‌سازی پنل از دسترس عموم
- 🌐 پنل وب حرفه‌ای با پشتیبانی از لاگ‌های رنگی (ANSI)، بکاپ و بازگردانی دیتابیس، و رابط خط فرمان برای دسترسی آسان
- 🗃️ ذخیره‌سازی تنظیمات در پایگاه داده SQLite همراه با سیستم خودترمیمی (Self-Healing) برای جلوگیری از خرابی دیتابیس
